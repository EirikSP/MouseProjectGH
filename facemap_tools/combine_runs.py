...
def combine(exp_names, results_folder, behavior_folder):
    """
    ...
    Args:
        ...
        behavior_folder - The global path to the folder containing the behaviour data.\n
    ...
    num_points = len(np.load(os.path.join(behavior_folder, experiment_base + '_negative.npy'), allow_pickle=True))
...
def combine_results_folder(results_folder, behavior_folder="D:\\mec-lec-por-data"):
    """
    ...
    while len(files)>0:
        ...
        complete_area, complete_area_smooth = combine(exp_filenames, results_folder, behavior_folder)
...
if __name__=='__main__':
    ...
    try:
        behavior_folder = sys.argv[2]
        combine_results_folder(results_folder, behavior_folder)
    except:
